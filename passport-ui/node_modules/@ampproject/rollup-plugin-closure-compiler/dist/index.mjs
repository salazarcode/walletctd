...
async function writeTempFile(content, extension = '', stableName = true) {
    ...
    const fullpath = join(tmpdir(), hash + extension);
...
async function parse(fileName, source) {
    try {
        return parse$1(source, DEFAULT_ACORN_OPTIONS);
    }
    catch (e) {
 ...
 * @param path
 */
function shouldRemoveStrictModeDeclarations(pluginOptions, outputOptions, path) {
